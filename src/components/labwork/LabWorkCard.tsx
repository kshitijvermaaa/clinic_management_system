...
import { LabWorkPaymentDialog } from './LabWorkPaymentDialog';
...
export const LabWorkCard: React.FC<LabWorkCardProps> = ({ labWork, onEdit, onViewFiles }) => {
  const { deleteLabWork, updateLabWork, refreshLabWork } = useLabWork();
  ...
  const handlePaymentUpdate = async () => {
    // Refresh the lab work data to get updated payment information
  ...
  const totalPaid = labWork.total_paid || 0;
  const balanceRemaining = labWork.balance_remaining || 0;

  const getPaymentStatusColor = () => {
    if (balanceRemaining > 0) return 'text-red-600';
    if (balanceRemaining < 0) return 'text-orange-600';
    return 'text-green-600';
  };

  const getPaymentStatusLabel = () => {
    if (balanceRemaining > 0) return 'Outstanding';
    if (balanceRemaining < 0) return 'Overpaid';
    return 'Paid';
  };
  ...
  return (
    <>
            ...
            <div className="space-y-2">
              <div className="text-sm">
                <div className="flex items-center gap-2 text-slate-600 mb-1">
                  ...
                  <span className="text-xs font-medium">Payment Summary</span>
                </div>
                <div className="grid grid-cols-3 gap-2 text-xs">
                  ...
                  <div className={`p-2 rounded ${balanceRemaining > 0 ? 'bg-red-50' : balanceRemaining < 0 ? 'bg-orange-50' : 'bg-green-50'}`}>
                    <div className={`font-medium ${getPaymentStatusColor()}`}>
                      {getPaymentStatusLabel()}
                    </div>
                    <div className={`font-bold ${getPaymentStatusColor()}`}>
      ...
      <LabWorkPaymentDialog
        open={showPaymentDialog}
        onOpenChange={setShowPaymentDialog}
        labWorkId={labWork.id}
        labWorkTitle={`${labWork.lab_type} - ${labWork.patients?.full_name}`}
        totalCost={totalCost}
